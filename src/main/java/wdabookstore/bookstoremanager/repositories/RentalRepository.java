...
public interface RentalRepository extends JpaRepository<RentalEntity, Long> {
    ...
    @Query(value = "SELECT * FROM tb_rentals WHERE returndate IS NOT NULL AND deleted = false", nativeQuery = true)
    List<RentalEntity> findHandedOutRentals();

    @Query(value = "SELECT * FROM tb_rentals WHERE deleted = true", nativeQuery = true)
    List<RentalEntity> findDeletedRentals();

    @Query(value = "SELECT * FROM tb_rentals WHERE returndate IS NULL AND deleted = false", nativeQuery = true)
    ...
    @Query(value = "SELECT EXISTS (SELECT 1 FROM tb_rentals WHERE user_entity_id = :userId AND book_entity_id = :bookId AND deleted = false)", nativeQuery = true)
